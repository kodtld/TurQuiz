...
def update_answerank(user_id, amount, level):
        new_rank = ("UPDATE Answerank SET level=:level, amount=:amount WHERE user_id=:user_id")
        db.session.execute(new_rank, {"level": level, "amount":amount,"user_id": user_id})
        db.session.commit()
